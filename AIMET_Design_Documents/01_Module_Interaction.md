...
主控制器模式的特点：
- 🎯 **QuantizationSimModel**: 红色标注的主控制器，统一协调所有模块
- 🔵 **核心模块**: 蓝色标注的核心组件，提供主要功能
- 🟢 **工具模块**: 绿色标注的辅助工具，提供支撑服务
- 🔗 **组合关系**: 箭头表示模块间的依赖和调用关系
...
![模块交互序列图](./images/module_interaction_sequence.svg)

时序图展示了AIMET系统从初始化到编码计算完成的完整交互过程：

**初始化阶段**: 用户创建QuantizationSimModel，系统依次创建各级组件并建立依赖关系

**编码计算阶段**: 通过前向传播收集统计信息，每个张量量化器更新其编码分析器的统计数据

**参数计算阶段**: 基于收集的统计信息计算量化编码参数，并返回给对应的量化器
