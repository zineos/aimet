...
### 2.1 类层次结构
**查看EncodingAnalyzer类层次结构图**: 在 [architecture_diagrams.html](./architecture_diagrams.html) 中的"EncodingAnalyzer 类层次结构"部分可以看到：

- 🟣 **抽象基类**: EncodingAnalyzer用紫色虚线框和斜体字表示
- 🟢 **具体实现**: 各种分析器实现用绿色表示
- 🔵 **组件模块**: 统计收集器、编码计算器等用蓝色表示
- 🟠 **工具模块**: 直方图收集器、KL散度计算器等用橙色表示
- 🔗 **组合关系**: 清晰展示各组件间的组合关系
- 💡 **功能说明**: 悬停查看每个类的详细功能描述
